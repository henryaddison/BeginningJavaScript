...
var isUser = function (user) {
  return typeof(user) === "object" && typeof user.name === "string" && typeof user.screen_name === "string" && Object.keys(user).length === 2;
};
...
var userWithTweetsToDiv = function (user) {
  var tweetList = toTagString('ul', user.tweets.map(function(tweet)  { return toTagString('li', tweet) }).join(''));
  return toTagString('div', toTagString('h1', user.name)+toTagString('h2', user.screen_name)+tweetList);
};
